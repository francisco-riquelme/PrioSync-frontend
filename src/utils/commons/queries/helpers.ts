...
/**
 * Retrieves only existing query factories without auto-initialization.
 *
 * Convenience wrapper around ClientManager.getInstance().getExistingQueryFactories()
 * that returns only entities that have already been initialized, without creating
 * new ones for missing entities.
 *
 * @template TTypes - Record of all available Amplify model types
 * @template TSelected - Selected entity names as string literals
 * @param config - Configuration for existing factory retrieval
 * @param config.entities - Array of entity names to check for existing factories
 * @param config.clientKey - Optional client key for isolation (default: "default")
 * @returns Object with query factories for entities that exist (may be partial)
 *
 * @example
 * ```typescript
 * const existingQueries = getExistingQueryFactories({
 *   entities: ["User", "Post", "Comment"],
 *   clientKey: "main"
 * });
 *
 * if (existingQueries.User) {
 *   const user = await existingQueries.User.get({ input: { userId: "123" } });
 * }
 * ```
 */
export function getExistingQueryFactories<
  TTypes extends Record<string, AmplifyModelType>,
  TSelected extends keyof TTypes & string,
>(config: {
  entities: readonly TSelected[];
  ...
  const { clientKey = "default", ...rest } = config;
  const manager = ClientManager.getInstance(clientKey);
  return manager.getExistingQueryFactories<TTypes, TSelected>(rest);
}

/**
 * Checks if specific entities have already been initialized.
 *
 * Convenience wrapper around ClientManager.getInstance().areEntitiesInitialized()
 * that checks whether all specified entities have query factories available.
 *
 * @param entities - Array of entity names to check for initialization
 * @param clientKey - Optional client key for isolation (default: "default")
 * @returns True if all entities are initialized, false otherwise
 *
 * @example
 * ```typescript
 * if (areEntitiesInitialized(["User", "Post"], "main")) {
 *   // Safe to use existing factories
 *   const queries = getExistingQueryFactories({
 *     entities: ["User", "Post"],
 *     clientKey: "main"
 *   });
 * } else {
 ...
 * ```
 */
export function areEntitiesInitialized(
  entities: readonly string[],
  clientKey: string = "default"
): boolean {
  const manager = ClientManager.getInstance(clientKey);
  return manager.areEntitiesInitialized(entities);
}

/**
 * Retrieves all initialized query factories for a specific client.
 *
 * Convenience wrapper around ClientManager.getInstance().getAllQueryFactories()
 * that returns all query factories that have been created for the specified client.
 *
 * @param clientKey - Optional client key for isolation (default: "default")
 ...
 * @example
 * ```typescript
 * const allFactories = getAllQueryFactories("main");
 * console.log(`Initialized entities: ${Object.keys(allFactories).join(", ")}`);
 * ```
 */
export function getAllQueryFactories(
  clientKey: string = "default"
): Record<string, unknown> {
  const manager = ClientManager.getInstance(clientKey);
  return manager.getAllQueryFactories();
}
//#endregion
