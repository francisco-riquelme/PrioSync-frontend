...
export default function GenerateFinalQuizButton({
  ...
  const crearHook = useCrearQuestionarioFinal({ onSuccess });
  ...
  const handleGenerate = async () => {
    try {
      ...
      if (onNotify) onNotify('Cuestionario final generado correctamente (20 preguntas de todos los módulos)', 'success');
    } catch (err) {
